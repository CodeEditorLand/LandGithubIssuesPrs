...
class Issue extends TreeItem {
	constructor(
		label: string,
		public query: { remote: GitRemote; assignee: string | undefined },
		public item: any
	) {
		super(label);
	}
...
export class GitHubIssuesPrsProvider implements TreeDataProvider<TreeItem> {
	...
	private async createIssue() {
		...
		const callback = async (
			selectedRemote: RemoteQuickPickItem | undefined
		) => {
			if (!selectedRemote) {
				return;
			}
			...
			if (
				selectedRemote.remote.username &&
				selectedRemote.remote.password
			) {
				github.authenticate({
					type: "basic",
			...
			await env.openExternal(
				Uri.parse(data.data.html_url + "/issues/new")
			);
		};
	...
	private async fetchChildren(element?: TreeItem): Promise<TreeItem[]> {
		...
		for (const issue of issues) {
			...
			if (!milestone) {
				...
				m &&
				m.due_on &&
				!(milestone.item && milestone.item.due_on)
			) {
				milestone.item = m;
			}
	...
	private async fetchAllIssues(
		...
		return items
			.map((item: any) => ({
				item,
				remote: remotes.find((remote) =>
					item.repository_url
						.toLowerCase()
						.endsWith(
							`/${remote.owner.toLowerCase()}/${remote.repo.toLowerCase()}`
						)
				),
			}))
	...
	private async openMilestone(milestone: Milestone) {
		...
		const url = `https://github.com/issues?utf8=%E2%9C%93&q=is%3Aopen+${
			item.milestone
				? `milestone%3A%22${item.milestone.title}%22`
				: "no%3Amilestone"
		}${assignee ? "+assignee%3A" + assignee : ""}`;
	...
	private async copyMarkdown(issue: Issue) {
		return env.clipboard.writeText(
			`[#${issue.item.number}](${issue.item.html_url})`
		);
	}
